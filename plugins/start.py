...
async def start(client, message):
    ...
    __I am a file renamer bot. Please send any telegram 
    **Document, Audio, or Video** and enter a new filename to rename it.__
...
async def send_doc(client, message):
    ...
    try:
        ...
        await message.reply_text("The database has been Cleared. Click on /start")
        return
    ...
    if user_type == "Free":
        LIMIT = 600
        await message.reply_text(f"Complete The Task and get Free subscription per day. Watch the full video https://lntechnical.works/{message.from_user.id} ")
        return
    ...
    if left > 0:
        ...
        if remain < int(file.file_size):
            await message.reply_text(
                f"Sorry! I can't upload files that are larger than {humanbytes(limit)}. File size detected {humanbytes(file.file_size)}\nUsed Daily Limit {humanbytes(used)} If you want to rename a large file, upgrade your plan ",
